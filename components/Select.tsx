...
interface IProps {
  ...
  sizes: number[],
  onSelectDough: (val: any) => void,
  onSelectSize: (val: any) => void,
}
    ...
    <div className="flex flex-col my-2">
      ...
      <ul className="inline-flex items-center relative h-10 p-1 space-x-1 bg-gray-200 rounded-md font-semibold text-blue-600 ">
        {sizes.map((size, i) => <li className="relative w-full" key={i}>
          <input className="sr-only peer" type="radio" value={size} name="size" id={idList[i]} onChange={(e) => onSelectSize(e.target.value)}/>
